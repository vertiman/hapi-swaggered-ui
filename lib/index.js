...
exports.register = (plugin, options, next) => {
  ...
  const internals = {
    handler (request, reply) {
            const hapiSwaggeredSettings = Hoek.reach(plugin, 'plugins.@resolver/hapi-swaggered.settings')
      ...
      const context = {
                ...
                authorization: _.clone(settings.authorization),
                version: packageInfo.version
            }

            if (request.query.token) {
                context.authorization.defaultValue = request.query.token;
            }
